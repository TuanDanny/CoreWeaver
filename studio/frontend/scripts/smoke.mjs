...
if (!html.includes("SWARM AI STUDIO V6.5")) {
  throw new Error("Browser title must match V6.5 shell");
}
...
for (const needle of ["Project", "Agents", "Logs", "Plan Review", "Settings", "Artifacts", "Debug Bundle", "Future Wiki", "Coming soon: Agent6 not implemented yet"]) {
  if (!app.includes(needle)) throw new Error(`Missing practical sidebar item or reason: ${needle}`);
}
...
for (const needle of ["swarm.timelineWidth", "swarm.rightWidth", "requestAnimationFrame", "splitter"]) {
  if (!app.includes(needle)) throw new Error(`Missing resizable log layout contract: ${needle}`);
}
if (!app.includes("minmax(280px,1fr)") || !app.includes("minmax(320px,${rightWidth}px)")) {
