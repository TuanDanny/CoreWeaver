...
TOOL_ALIASES = {
    # OSS CAD Suite on Windows may ship `verilator` as a shell shim that cmd.exe
    # cannot execute directly, while the real executable is `verilator_bin.exe`.
...
def _detect_command(name: str, command: list[str]) -> dict[str, Any]:
    commands = [command] + TOOL_ALIASES.get(name, [])
    last_missing = f"{command[0]} not found on PATH"
    for candidate in commands:
        result = _detect_one_command(candidate)
        if result.get("available"):
            result["command"] = candidate[0]
            return result
        last_missing = str(result.get("reason") or last_missing)
    return {"available": False, "path": None, "version": "", "reason": last_missing}


def _detect_one_command(command: list[str]) -> dict[str, Any]:
